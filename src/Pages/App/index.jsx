...
const AppRoutes = () => {
  ...
  let routes = useRoutes([
    ...
    { path: '/dashboard/user', element: <PrivateRoute element={<><Navbar /><DashBoard userType="user" /></>} /> },
    // { path: '/dashboard/admin', element: <PrivateRoute element={<><Navbar /><KanbanTable feedbacks={dummyFeedbacks} /></>} requiredRole="admin" /> },
    { path: '/confirmacion', element: <><Navbar /><Confirmation /></> },
    { path: '/piso', element: <><Navbar /><FloorMap /></> },
    { path: '/*', element: <><Navbar /><NotFound /></> },
    // { path: '/piso-12', element: <><Navbar /><FloorMap /></> },
    { path: '/dashboard/admin', element: <div className="font-medium min-h-screen flex flex-col"><Navbar/><div className="flex-1 overflow-y-auto"><KanbanBoardPage /></div></div> },
  ]);
